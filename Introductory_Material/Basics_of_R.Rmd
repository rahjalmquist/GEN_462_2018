...
setwd("insert_path_here") #this allows you to set the working directory. An example path would be "~/Desktop/GEN_462_2018/Introductory_Material/"
```

3. How to load a dataset  
```{r}
#loading a dataset into R requires that you are in the correct directory.
read.table("exampletextfile.txt", header = TRUE, sep = "\t") #load an example text file in your Introductory_Materials folder; results appear below. sep refers to the type of separator that delimits data in the file, which is a tab for this example file.

#To save the data file to continue working it, you can assign it a name, like so:
